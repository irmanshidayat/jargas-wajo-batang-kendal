...
Write-Host ""
Write-Host "Step 2: Rebuild Docker containers (development)..." -ForegroundColor Green
$rebuildCmd = "ssh ${Username}@${ServerIP} 'cd $ProjectPath && docker-compose -f docker-compose.dev.yml build --no-cache'"
try {
...
$stopCmd = "ssh ${Username}@${ServerIP} 'cd $ProjectPath && docker-compose -f docker-compose.dev.yml down'"
...
$startCmd = "ssh ${Username}@${ServerIP} 'cd $ProjectPath && docker-compose -f docker-compose.dev.yml up -d'"
try {
...
$migrationCheckCmd = "ssh ${Username}@${ServerIP} 'cd $ProjectPath && docker-compose -f docker-compose.dev.yml logs backend --tail 50 | grep -i migration || docker-compose -f docker-compose.dev.yml logs backend --tail 50 | Select-String -Pattern \"migration\" -CaseSensitive:`$false'"
try {
...
$verifyCmd = "ssh ${Username}@${ServerIP} 'cd $ProjectPath && docker-compose -f docker-compose.dev.yml exec -T mysql mysql -u root -padmin123 jargas_apbn_dev -e \"SHOW TABLES;\" 2>/dev/null | head -20'"
try {
...
Write-Host "   ssh ${Username}@${ServerIP} 'cd $ProjectPath && docker-compose -f docker-compose.dev.yml logs backend --tail 100'" -ForegroundColor Gray
Write-Host ""
Write-Host "2. Jika migration belum berjalan, jalankan manual:" -ForegroundColor White
Write-Host "   ssh ${Username}@${ServerIP} 'cd $ProjectPath && docker-compose -f docker-compose.dev.yml exec backend alembic upgrade head'" -ForegroundColor Gray
Write-Host ""
