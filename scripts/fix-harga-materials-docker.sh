...
# Script untuk memperbaiki kolom harga di tabel materials di database VPS (via Docker)
# FIX UNTUK ERROR: Unknown column 'materials.harga' in 'field list'
# Usage: ./fix-harga-materials-docker.sh
...
echo -e "${CYAN}========================================${NC}"
echo -e "${CYAN}Fix Kolom harga di Tabel materials${NC}"
...
SQL_FILE="$SCRIPT_DIR/fix_harga_materials.sql"
...
if docker exec -i "${DB_CONTAINER}" mysql -u"${DB_USER}" -p"${DB_PASSWORD}" "${DB_NAME}" < "$SQL_FILE"; then
    ...
    echo -e "${GREEN}   Kolom harga sudah ditambahkan ke tabel materials.${NC}"
    echo ""
    echo -e "${CYAN}📋 Kolom yang ditambahkan:${NC}"
    echo -e "   - materials.harga (DECIMAL(15,2) NULL)"
    echo ""
    echo -e "${GREEN}🎉 Sekarang fitur tambah barang seharusnya sudah berfungsi!${NC}"
    exit 0
else
    echo ""
